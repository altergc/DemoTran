...
for (numYear in yearList) {
    ...
    #####################
    dfMerge <- dfMerge %>% filter(!is.na(dfMerge$'GDPpc') & !is.na(dfMerge$CBR) )
    
    ##################################
    selTitle <- paste("Income by Crude Birth Rate", txtYear, sep=" ")
    xmin <- 0
    xmax <- 20000
    ...
    #     ggtitle(selTitle) +
    #     geom_point( aes(y=CBR ) ) +
    #     scale_x_continuous(name="GDPpc", limit=c(xmin, xmax)  )+
    #     scale_y_continuous(name="Crude Birth Rate" ) 
    # )
    
    ###### OLS
    dfMerge$lExp <- log( (100/(100-dfMerge$CBR)) - 1)
    ...
    dfMerge$PredE <- -(100 - 100*(1 + exp(dfMerge$Predicted_Y)))/(1 + exp(dfMerge$Predicted_Y))
...
ggplot(dfPredE0, mapping=aes(x=GDPpc , y=PredE , group=year, color=year )) +
  ggtitle("Income by Crude Birth Rate") +
