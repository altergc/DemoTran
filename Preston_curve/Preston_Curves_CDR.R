...
for (numYear in yearList) {
    ...
    #####################
    dfMerge <- dfMerge %>% filter(!is.na(dfMerge$'GDPpc') & !is.na(dfMerge$CDR) )
    
    ##################################
    selTitle <- paste("Income by Crude Death Rate", txtYear, sep=" ")
    xmin <- 0
    xmax <- 20000
...
ggplot(dfPredE0, mapping=aes(x=GDPpc , y=PredE , group=year, color=year )) +
  ggtitle("Income by Crude Death Rate") +
