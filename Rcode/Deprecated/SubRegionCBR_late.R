...
###### Select countries with milestones CBR= 35 and 25, not 15 ############
...
write.xlsx(dfBD, file="Output/CBRlate.xlsx", sheetName="CBR35-15")
...
write.xlsx(dft3, file="Output/CBRlate.xlsx", sheetName="CountryNames", append=TRUE)
...
write.xlsx(subRegionMean, file="Output/CBRlate.xlsx", sheetName="means_CBR35-15", append=TRUE)
...
for(vv in c("CBR"))   {
  ...
  subSelect <- list(   "Northern Africa", "Southern Africa", "Central America", 
  ...
  ##### save plot to png  ############
  pngFile <- paste("Output/CBRpng/milestonesSub", vv, "_late.png", sep="")
  png(pngFile)
